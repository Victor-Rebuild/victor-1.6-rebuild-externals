...
inline void AkUInt32ToCpuSet(AkUInt32 cpu_mask, cpu_set_t *cpu_set)
{
...
inline int SetAffinityMask(AkUInt32 cpu_mask)
{
	if (cpu_mask == 0) {
		return 0;
	}
	cpu_set_t cpu_set;
	AkUInt32ToCpuSet(cpu_mask, &cpu_set);
	return pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpu_set);
}


#define AK_THREAD_INIT_CODE(_threadProperties) SetAffinityMask(_threadProperties.dwAffinityMask)
